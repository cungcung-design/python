...
class SafeListFrame(tk.Frame):
    ...
    def setup_ui(self):
        ...
        tk.Label(
            left_frame, text="POS SALES", font=("Arial", 15, "bold"), bg="white"
        ).pack(pady=10)

        tk.Label(
            left_frame, text="Barcode Search:", font=("Arial", 12, "bold"), bg="white"
        ...
        tk.Button(search_frame, text="Search", bg="green", fg="white").pack(
            side="left", padx=5
        )

        tk.Label(left_frame, text="Item:", font=("Arial", 10, "bold"), bg="white").pack(
            anchor="w", pady=5
        )
        ...
        right_frame.grid(
            row=0, column=1, sticky="nsew", pady=(20, 0)
        )  # top margin 20px

        tk.Label(
            right_frame, text="Cart:", font=("Arial", 10, "bold"), bg="white"
        ).pack(anchor="w", pady=5)

        cart_columns = ("Name", "Price", "Qty", "Total")
        cart_tree = ttk.Treeview(
            right_frame, columns=cart_columns, show="headings", height=15
        )
        ...
        tk.Label(
            total_frame, text="Total:", font=("Arial", 10, "bold"), bg="white"
        ).pack(side="left")
        tk.Label(
            total_frame, text="$0.00", font=("Arial", 10, "bold"), bg="white"
        ).pack(side="left", padx=5)

        # -------- Staff --------
        tk.Label(
            right_frame, text="Staff:", font=("Arial", 10, "bold"), bg="white"
        ).pack(anchor="w")
        ...
        tk.Button(
            checkout_frame, text="Check out", bg="green", fg="white", width=18
        ).pack(side="left", padx=5)
        tk.Button(
            checkout_frame, text="Print Receipt", bg="lightblue", fg="white", width=18
        ).pack(side="left", padx=5)
